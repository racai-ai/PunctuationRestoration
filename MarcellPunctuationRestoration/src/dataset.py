...
def parse_data(file_path, tokenizer, sequence_len, token_style, mode='file'):
    """
    ...
    while idx < len(lines):
            ...
            if mode=='file':
                ...
                y_mask = [0]  # which positions we need to consider while evaluating i.e., ignore pad or sub tokens
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, files, tokenizer, sequence_len, token_style, is_train=False, augment_rate=0.1,
                 augment_type='substitute',mode='file'):
        """
        ...
        if isinstance(files, list):
            self.data = []
            for file in files:
                self.data += parse_data(file, tokenizer, sequence_len, token_style, mode)
        else:
            self.data = parse_data(files, tokenizer, sequence_len, token_style, mode)
