...
elif args.language == 'romanian':
    train_set = Dataset(os.path.join(args.data_path, 'ro/marcell_corpus_train.data'), tokenizer=tokenizer, sequence_len=sequence_len,
                        token_style=token_style, is_train=True, augment_rate=ar, augment_type=aug_type)
    val_set = Dataset(os.path.join(args.data_path, 'ro/marcell_corpus_dev.data'), tokenizer=tokenizer, sequence_len=sequence_len,
                      token_style=token_style, is_train=False)
    test_set_ref = Dataset(os.path.join(args.data_path, 'ro/marcell_corpus_test.data'), tokenizer=tokenizer, sequence_len=sequence_len,
                           token_style=token_style, is_train=False)
    #test_set_asr = Dataset(os.path.join(args.data_path, 'en/test2011asr'), tokenizer=tokenizer, sequence_len=sequence_len,
    #                       token_style=token_style, is_train=False)
    #test_set = [val_set, test_set_ref, test_set_asr]
...
def train():
    ...
    log="Best validation Acc [%f] in epoch [%d]"%(best_val_acc,best_epoch)
    with open(log_path, 'a') as f:
        f.write(log)
    ...
    for loader in test_loaders:
        ...
        log = "Evaluating [%s]"%(loader.dataset.files)+"\n"+\
        ...
        with open(log_path, 'a') as f:
            f.write(log)
        log_text = ''
        for i in range(1, 1+len(punctuation_dict)):
            name="MicroAvg"
            if i<len(punctuation_dict):
                val_list=list(punctuation_dict.values())
                key_list=list(punctuation_dict.keys())
                pos=val_list.index(i)
                name=key_list[pos]
            log_text += name+"\t"+str(precision[i] * 100) + '\t' + str(recall[i] * 100) + '\t' + str(f1[i] * 100) + "\n"
        print(log_text)
        with open(log_path, 'a') as f:
            f.write(log_text[:-1] + '\n\n')
